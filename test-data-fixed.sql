-- ============================================================================
-- MAIA SSS APP - TEST DATA (FIXED VERSION)
-- ============================================================================
-- This version fixes the PostgreSQL syntax errors
-- ============================================================================
...
SELECT sofia.id, 'SEL', 2, 'OPEN', true, '2025-11-10', '2025-12-15', wendy.id, 'Student showing severe separation anxiety. Crying for extended periods.', 'PARENT', 'URGENT: Immediate Tier 2 intervention needed.', wendy.id, ARRAY['Individual counseling', 'Parent consultation']
FROM wendy, sofia
UNION ALL
SELECT lucas.id, 'ACADEMIC_SUPPORT', 1, 'OPEN', true, '2025-11-15', '2026-01-30', wendy.id, 'Student struggling with phonics and letter recognition.', 'BEHAVIOR_FORM', 'URGENT: Fast-track evaluation needed.', wendy.id, ARRAY['Reading intervention']
FROM wendy, lucas
UNION ALL
SELECT emma.id, 'ACADEMIC_SUPPORT', 2, 'OPEN', false, '2025-10-01', '2025-12-20', wendy.id, 'Student struggling with math concepts.', 'BEHAVIOR_FORM', 'Tier 2 small group instruction.', wendy.id, ARRAY['Math small group']
FROM wendy, emma
UNION ALL
SELECT ahmed.id, 'ACADEMIC_SUPPORT', 3, 'OPEN', false, '2025-09-15', '2026-06-15', wendy.id, 'Student has documented dyslexia.', 'ADMIN', 'IEP in place.', wendy.id, ARRAY['Wilson Reading Program']
FROM wendy, ahmed
UNION ALL
SELECT emma.id, 'SEL', 1, 'OPEN', false, '2025-11-01', '2025-12-01', wendy.id, 'Student having difficulty making friends.', 'BEHAVIOR_FORM', 'Started lunch bunch group.', wendy.id, ARRAY['Social skills group']
FROM wendy, emma
UNION ALL
SELECT isabella.id, 'SEL', 2, 'OPEN', false, '2025-10-15', '2026-02-15', wendy.id, 'Student showing signs of anxiety and perfectionism.', 'SELF', 'Weekly check-ins established.', wendy.id, ARRAY['Individual counseling']
FROM wendy, isabella
UNION ALL
SELECT lucas.id, 'BULLYING', 2, 'OPEN', false, '2025-11-12', '2025-12-10', wendy.id, 'Student reported being excluded by peers.', 'BEHAVIOR_FORM', 'Following bullying protocol.', wendy.id, ARRAY['Restorative practices']
FROM wendy, lucas
UNION ALL
SELECT ahmed.id, 'CHILD_PROTECTION', 3, 'OPEN', false, '2025-11-05', NULL, wendy.id, 'Student disclosed concerning information.', 'ADMIN', 'CONFIDENTIAL: Report filed.', wendy.id, ARRAY['Safety planning']
FROM wendy, ahmed
UNION ALL
SELECT isabella.id, 'CONFLICT_RESOLUTION', 1, 'OPEN', false, '2025-11-14', '2025-11-25', wendy.id, 'Conflict with peer in group project.', 'BEHAVIOR_FORM', 'Mediation scheduled.', wendy.id, ARRAY['Peer mediation']
FROM wendy, isabella;
...
SELECT c.id, 'ACADEMIC', 2, 'Math Facts Fluency Group', 'Small group multiplication instruction', '2025-10-08', '2025-12-20', 10, '3x per week', w.id, true
FROM cases c
JOIN students s ON s.id = c.student_id
CROSS JOIN wendy w
WHERE s.student_id = 'TEST-G3-001' AND c.case_type = 'ACADEMIC_SUPPORT'
UNION ALL
SELECT c.id, 'SEL', 2, 'Anxiety Management', 'Weekly individual counseling sessions', '2025-10-20', '2026-02-15', 16, 'Weekly, 45 min', w.id, true
FROM cases c
JOIN students s ON s.id = c.student_id
CROSS JOIN wendy w
WHERE s.student_id = 'TEST-MS-001' AND c.case_type = 'SEL' AND c.tier = 2
UNION ALL
SELECT c.id, 'ACADEMIC', 3, 'Wilson Reading System', 'Intensive phonics-based reading program', '2025-09-20', '2026-06-15', 36, 'Daily, 60 min', w.id, true
FROM cases c
JOIN students s ON s.id = c.student_id
CROSS JOIN wendy w
...
SELECT i.id, '2025-10-08', '10:00:00', 30, w.id, true, 'First session. Emma engaged well.', 'Good baseline.'
...
UNION ALL
SELECT i.id, '2025-10-10', '10:00:00', 30, w.id, true, 'Continued with array models.', 'Progress noted.'
FROM interventions i
JOIN cases c ON c.id = i.case_id
JOIN students s ON s.id = c.student_id
CROSS JOIN wendy w
WHERE s.student_id = 'TEST-G3-001' AND i.intervention_name = 'Math Facts Fluency Group'
UNION ALL
SELECT i.id, '2025-10-20', '14:00:00', 45, w.id, true, 'Initial session. Built rapport.', 'Good insight into triggers.'
FROM interventions i
JOIN cases c ON c.id = i.case_id
JOIN students s ON s.id = c.student_id
CROSS JOIN wendy w
...
SELECT s.id, c.id, '2025-11-13', '15:30:00', w.id, true, 'COMPLETED', 'Discuss Sofia separation anxiety', 'Parents committed to routine.', 'Daily check-in with teacher.'
FROM students s
JOIN cases c ON c.student_id = s.id
CROSS JOIN wendy w
WHERE s.student_id = 'TEST-PREK-001' AND c.is_urgent = true AND c.case_type = 'SEL'
UNION ALL
SELECT s.id, c.id, '2025-11-22', '16:00:00', w.id, true, 'SCHEDULED', 'Review progress in math intervention', NULL, NULL
FROM students s
JOIN cases c ON c.student_id = s.id
CROSS JOIN wendy w
WHERE s.student_id = 'TEST-G3-001' AND c.case_type = 'ACADEMIC_SUPPORT';
